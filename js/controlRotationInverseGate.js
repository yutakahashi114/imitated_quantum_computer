 ...
 */
const ControlRotationInverseGate = (input, control, target, maxBit, rot) => {
    ...
    for (let i = 0; i < maxValue; i++ ) {
        if (input[i][0] !== 0 || input[i][1] !== 0) {
            output[i] = input[i];
            if (ControlRotationInverseGateOneState(i, control, target, maxBit)) {
 ...
 */
const ControlRotationInverseGateOneState = (input, control, target, maxBit) => {
    // 入力を2進数表示
    let str_0 = '';
    for (i = 0; i < maxBit; i++) {
        str_0 += '0';
